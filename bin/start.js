...
exec(`mkdir ${process.argv[2]} && cd ${process.argv[2]} && npm init -f`, (initErr, initStdout, initStderr) => {
	...
	fs.readFile(packageJSON, (err, file) => {
		if (err) throw err;
		const data = file.toString().replace('"test": "echo \\"Error: no test specified\\" && exit 1"', scripts).replace('"keywords": []', babel);
	...
	https.get('https://raw.githubusercontent.com/Nikhil-Kumaran/reactjs-boilerplate/master/.gitignore', (res) => {
